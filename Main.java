...
public class Main {
    ...
    private static void ordenarAlunosRecursivo(int inicio, int fim) {
        if (inicio < fim) {
            int pivo = particionar(inicio, fim);
            ordenarAlunosRecursivo(inicio, pivo - 1);
            ordenarAlunosRecursivo(pivo + 1, fim);
        }
    }

    private static int particionar(int inicio, int fim) {
        Aluno pivo = catalogo.get(fim);
        int i = inicio - 1;

        for (int j = inicio; j < fim; j++) {
            if (catalogo.get(j).getNivel() > pivo.getNivel()) {
                i++;
                Aluno temp = catalogo.get(i);
                catalogo.set(i, catalogo.get(j));
                catalogo.set(j, temp);
            }
        }

        Aluno temp = catalogo.get(i + 1);
        catalogo.set(i + 1, catalogo.get(fim));
        catalogo.set(fim, temp);
    ...
    public static void inserirAluno(Scanner input){
        ...
        catalogo.add(aluno);
        ordenarAlunosRecursivo(0, catalogo.size() - 1);
        aluno.getInfo();
